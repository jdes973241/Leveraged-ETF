...
# 0. 頁面設定
# ==========================================
st.set_page_config(page_title="Dynamic Momentum (Dashboard + Backtest)", layout="wide")
...
@st.cache_data(ttl=3600, show_spinner="計算回測訊號...")
...
def run_strict_backtest(data, risk_weights, winners_series, safe_signals):
    ...
    for idx, i in enumerate(range(start_idx, len(dates))):
        ...
        # A. Signal (Monthly Locked Selection)
...
# 7. Dashboard 顯示
# ==========================================
st.title("🛡️ 雙重動能與動態風控 (Dashboard & Backtest)")
st.caption("架構: Monthly Lock Selection / Daily Risk / Strict T+1")
...
if not live_data.empty:
    ...
    # Detailed Tables
    t1, t2, t3 = st.tabs(["📊 動能排名", "🛡️ 避險輪動", "📈 風控詳情"])
    with t1: st.dataframe(sel_df.style.format("{:.2f}"), use_container_width=True)
    with t2: st.dataframe(safe_df.style.format("{:.2%}"), use_container_width=True)
    with t3:
        if winner in risk_live:
            r_show = risk_live[winner].tail(5)[['Price','Vol','Exit_Th','Entry_Th','Weight']]
            st.dataframe(r_show.style.format("{:.2f}"), use_container_width=True)
